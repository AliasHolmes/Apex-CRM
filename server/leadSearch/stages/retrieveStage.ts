...
export async function executeRetrieveStage(
  ...
  const executeTavilyLane = async (plans: { plan: (typeof roundPlans)[0]; index: number }[]) => {
    ...
    recordTrace({
      ...
      tavily: {
        ...
        includeDomains: Array.from(new Set(plans.flatMap(({ plan }) => plan.item.tavily.includeDomains || [])))
      },
      metadata: { discoveryProviderMode }
    });
    logEvent(`Round ${round}: executing ${plans.length}/${roundPlans.length} Tavily queries (mode=${discoveryProviderMode}).`);

    await runProviderQueue(plans.map(({ plan, index }) => ({
  ...
  const executeBrightDataLane = async (plans: { plan: (typeof roundPlans)[0]; index: number }[], waveLabel: string) => {
    if (plans.length === 0) return;

    usingBrightDataSearch = true;
    stats.sourceProvider = stats.sourceProvider === 'tavily' && roundItems.length === 0 ? 'brightdata_search' : 'mixed';
    logEvent(`Round ${round}: executing ${plans.length} Bright Data searches (${waveLabel}, mode: ${brightDataSearchMode}).`);

    const bdResults = await runProviderQueue(plans.map(({ plan, index }) => ({
        ...
        try {
          const results = await executeBrightDataSearchWithRetry(async attempt => {
            ...
            try {
              ...
              incrementCounter(state.brightDataStats.failureReasons, classified.reasonCode);
              if (classified.reasonCode === 'target_transient') state.brightDataStats.transientFailures++;
  ...
  const canAttemptBD = brightDataReady && !brightDataProviderDisabled && brightDataSearchMode !== 'off';
  ...
  const unconditionalBdPlans = canAttemptBD
    ? roundPlans
        .map((plan, index) => ({ plan, index }))
        .filter(({ plan }) => shouldRunBrightDataForTask(plan.item, discoveryProviderMode, bdSearchMode, {
  ...
  if (canAttemptBD) {
    ...
    const conditionalBdPlans = roundPlans
      .map((plan, index) => ({ plan, index }))
      .filter(({ plan, index }) => !executedBdIndices.has(index) && shouldRunBrightDataForTask(plan.item, discoveryProviderMode, bdSearchMode, {
        brightDataReady: true,
        tavilyResultCount
      }));
