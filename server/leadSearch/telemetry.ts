...
export type ProviderSummary = Record<
  string,
  {
    calls: number;
    successes: number;
...
const PROVIDER_KEYS: MiningProvider[] = [
  "llm",
  "tavily",
  "brightdata",
  "sqlite",
...
export function clampSearchLogRetentionLimit(
  raw = process.env.SEARCH_LOG_RETENTION_LIMIT,
): number {
...
export function estimateLLMCostUsd(inputTokens = 0, outputTokens = 0): number {
  const inputPerMillion = Number(
    process.env.LLM_OBSERVABILITY_INPUT_COST_PER_1M || 0,
  );
  const outputPerMillion = Number(
    process.env.LLM_OBSERVABILITY_OUTPUT_COST_PER_1M || 0,
  );
...
export class MiningTelemetryRecorder {
  ...
  constructor(
    ...
    const configured = Number(process.env.LEAD_TELEMETRY_MAX_EVENTS || 2000);
    this.maxEvents =
      Number.isFinite(configured) && configured > 0
        ? Math.min(Math.max(Math.floor(configured), 500), 20_000)
        : 2000;
  }
  ...
  record(
    ...
    if (
      llmStageLogger &&
      (traceEvent.provider === "llm" || traceEvent.llm) &&
  ...
  /**
   * Live event buffer. Returns the internal array reference (do not mutate).
   * Bounded by LEAD_TELEMETRY_MAX_EVENTS (default 2000) - oldest events are
  ...
  getSummary(): MiningTraceSummary {
    ...
    const costSummary: CostSummary = {
      ...
      costPerAcceptedLead:
        returned > 0
          ? Number((llm.estimatedCostUsd / returned).toFixed(6))
          : undefined,
      tokensPerAcceptedLead:
        returned > 0 ? Math.round(llm.totalTokens / returned) : undefined,
    };
...
function sanitizeMetadata(metadata: Record<string, any>) {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    ...
    if (typeof value === "string")
      sanitized[key] = sanitizeTelemetryString(value, 500);
...
export function setLlmStageLogger(
  logger: ((entry: LlmStageLogEntry) => void) | null,
) {
