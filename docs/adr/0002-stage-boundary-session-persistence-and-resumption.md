...
- `POST /api/find-leads` supports a transition mode:
  - With `?mode=job` or `Prefer: respond-async`, the endpoint immediately responds with `202 Accepted` and `{ sessionId, status: 'running', streamUrl: '/api/mining-sessions/:id/stream' }`.
